...
class DataScienceScen(Scenario):
    ...
    def _get_description_mle_format(self, hardware: str = "a GPU", time_limit: int = 24, step_limit: int = 500):
        """
        Get competition description in MLE-bench format.

        This method constructs the full MLE-bench prompt by concatenating:
        1. Base MLE-bench instructions
        2. Agent-specific additional notes (hardware, time, step limits)
        3. Competition-specific description

        Args:
            hardware: Hardware description (e.g., "a GPU", "4 CPUs")
            time_limit: Maximum runtime in hours
            step_limit: Maximum number of steps

        Returns:
            str: Full MLE-bench formatted description
        """
        # 1. Load base MLE-bench instructions
        ...
        if not instructions_path.exists():
            logger.warning(f"MLE-bench instructions not found at {instructions_path}, using standard format")
            return self._get_description()

        base_instructions = instructions_path.read_text()

        # 2. Add agent-specific additional notes
...
"""
        ...
        # 3. Get competition-specific description
        comp_desc_path = Path(f"{DS_RD_SETTING.local_data_path}/{self.competition}/description.md")
        ...
        if comp_desc_path.exists():
            ...
            competition_description = self._get_description()
            if isinstance(competition_description, dict):
                competition_description = json.dumps(competition_description, indent=2)

        # 4. Concatenate all sections with proper formatting
...
"""
        ...
        logger.info(f"Generated MLE-bench format description for {self.competition}")
